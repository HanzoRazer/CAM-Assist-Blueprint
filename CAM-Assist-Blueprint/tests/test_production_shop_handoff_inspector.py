"""
Phase 6 tests for CAM-A20 Production Shop Handoff — inspector presence rendering.
...
Witnessed:
- detection via the conventional production_shop/ path
- detection via explicit --handoff
- not-declared when absent
- detection only: a handoff with unparseable contents is still "present"
  (the inspector never parses, validates, resolves, or completeness-checks it)
- explicit --handoff to a missing path -> exit 2
- JSON output carries production_shop_handoff (presence + path)
...
def write_conventional_handoff(parent: Path, pkg_name: str, content: str = '{"x": 1}') -> Path:
    d = parent / "production_shop"
...
def test_handoff_present_via_conventional_path(tmp_path):
    pkg = make_package(tmp_path)
    write_conventional_handoff(tmp_path, "pkg")
    code, out, _err = run_inspect(pkg)
    assert "Production Shop Handoff:" in out
    assert "Production Shop Handoff:\n  present" in out


def test_handoff_not_declared_when_absent(tmp_path):
    pkg = make_package(tmp_path)
    code, out, _err = run_inspect(pkg)
    assert "Production Shop Handoff:\n  not declared" in out


def test_committed_example_package_shows_handoff_present():
...
def test_handoff_present_via_explicit_flag(tmp_path):
    pkg = make_package(tmp_path)
    h = tmp_path / "somewhere_handoff.json"
    h.write_text('{"x": 1}', encoding="utf-8")
    code, out, _err = run_inspect(pkg, "--handoff", h)
    assert "Production Shop Handoff:\n  present" in out


def test_explicit_missing_handoff_returns_2(tmp_path):
    pkg = make_package(tmp_path)
    code, _out, err = run_inspect(pkg, "--handoff", tmp_path / "nope_handoff.json")
    assert code == 2
    assert "not found" in err.lower()


# ---------------------------------------------------------------------------
# Detection only — never parses/validates the handoff
# ---------------------------------------------------------------------------

def test_unparseable_handoff_still_present(tmp_path):
    pkg = make_package(tmp_path)
    # Garbage content: if the inspector parsed/validated it, this would error.
    write_conventional_handoff(tmp_path, "pkg", content="this is not json {{{")
    code, out, _err = run_inspect(pkg)
    assert "Production Shop Handoff:\n  present" in out
...
def test_json_output_carries_production_shop_handoff(tmp_path):
    pkg = make_package(tmp_path)
    h = write_conventional_handoff(tmp_path, "pkg")
    code, out, _err = run_inspect(pkg, "--json")
    data = json.loads(out)
    assert data["production_shop_handoff"]["present"] is True
    assert data["production_shop_handoff"]["path"] == str(h)


def test_json_output_handoff_absent(tmp_path):
    pkg = make_package(tmp_path)
    code, out, _err = run_inspect(pkg, "--json")
    data = json.loads(out)
    assert data["production_shop_handoff"]["present"] is False
    assert data["production_shop_handoff"]["path"] is None
